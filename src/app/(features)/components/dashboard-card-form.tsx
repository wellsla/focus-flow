...
import { DashboardCard, DashboardCardVisualStyle, DashboardCardFeature, ApplicationStatus, RoutinePeriod, GoalStatus, GoalTimeframe } from "@/lib/types";
import { Textarea } from "@/components/ui/textarea";
import { Command, CommandEmpty, CommandGroup, CommandInput, CommandItem, CommandList } from "@/components/ui/command";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { cn } from "@/lib/utils";
import { AlertCircle, Briefcase, CalendarCheck, Check, Clock, DollarSign, Euro, FileText, Flag, ShieldAlert, Target, TrendingUp, Wallet, XCircle, ChevronsUpDown } from "lucide-react";
...
const visualizationOptions: { value: DashboardCardVisualStyle, label: string }[] = [
...
const iconOptions = Object.keys(iconComponents).map(name => ({ value: name, label: name, icon: iconComponents[name] }));

const applicationStatusOptions: ApplicationStatus[] = ['Applied', 'Interviewing', 'Offer', 'Rejected', 'Wishlist'];
const routinePeriodOptions: RoutinePeriod[] = ['morning', 'afternoon', 'evening'];
const goalStatusOptions: GoalStatus[] = ['Not Started', 'In Progress', 'Achieved'];
const goalTimeframeOptions: GoalTimeframe[] = ['Short-Term', 'Mid-Term', 'Long-Term'];
...
const formSchema = z.object({
  ...
  metric: z.string(),
  applicationStatus: z.enum(['Applied', 'Interviewing', 'Offer', 'Rejected', 'Wishlist']).optional(),
...
export function DashboardCardForm({ card, onSubmit, onDelete }: DashboardCardFormProps) {
    ...
    useEffect(() => {
        ...
        form.setValue('metric', metricOptionsMap[feature][0].value);
    ...
    useEffect(() => {
        // Auto-generate title and subtext when metric changes, if it's a new card
        if (!card) {
            const selectedMetric = metricOptionsMap[feature].find(m => m.value === metric);
            if (selectedMetric) {
                form.setValue('title', selectedMetric.label);
                form.setValue('subtext', `Dynamic card showing: ${selectedMetric.label}`);
            }
        }
    }, [feature, metric, card, form]);


  function handleSubmit(values: z.infer<typeof formSchema>) {
    const { title, subtext, icon, visualization, feature, metric, applicationStatus, routinePeriod, goalStatus, goalTimeframe } = values;
    onSubmit({
        title,
        subtext: subtext || '',
        icon,
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
         ...
         <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder="A short description for the card." {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
                                    ...
                                    {field.value
                                        ? (
                                            <div className="flex items-center gap-2">
                                               {React.createElement(iconComponents[field.value] || AlertCircle, {className: "h-4 w-4"})}
                                                {iconOptions.find((icon) => icon.value === field.value)?.label}
                                            </div>
                                        )
                                        : "Select icon"}
                                        ...
                                        {iconOptions.map((icon) => (
                                            ...
                                            >
                                                ...
                                                <Check
                                                    className={cn(
                                                        "ml-auto h-4 w-4",
                                                        field.value === icon.value ? "opacity-100" : "opacity-0"
                                                    )}
                                                />
                                            </CommandItem>
                                        ))}
        ...
        <div className="flex justify-between">
            <Button type="submit" className="flex-grow">{card ? 'Update Card' : 'Add Card'}</Button>
            {card && onDelete && (

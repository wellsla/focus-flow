...
import { DashboardCard, JobApplication, Task, TimeTrackingEntry, FinancialAccount, IncomeSettings, Currency, Goal } from "@/lib/types";
import { AlertCircle, Briefcase, CalendarCheck, Clock, DollarSign, Euro, FileText, Flag, ShieldAlert, Target, TrendingUp, Wallet, XCircle, ChevronDown, ChevronUp } from "lucide-react";
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "@/components/ui/collapsible";
import { cn } from "@/lib/utils";
import React, { useState, ElementType, useEffect } from "react";
import { Button } from "@/components/ui/button";
import { startOfWeek, parseISO, differenceInSeconds } from 'date-fns';
...
function getCardData(card: DashboardCard, allData: DynamicDashboardCardProps['allData']) {
    const { config } = card;
    const { jobApplications, tasks, timeTrackingEntries, financialAccounts, incomeSettings, goals } = allData;
    ...
    switch(config.feature) {
        case 'applications':
            if(config.metric === 'status' && config.applicationStatus) {
                const filteredApps = jobApplications.filter(app => app.status === config.applicationStatus);
                value = filteredApps.length.toString();
                details = filteredApps.map(app => ({ title: app.company, label: app.role }));
            } else { // 'total'
        ...
        case 'goals':
            ...
            if (config.metric === 'status' && config.goalStatus) {
                ...
                filteredGoals = goals.filter(g => g.timeframe === config.goalTimeframe);
            } else { // 'total'
                filteredGoals = goals;
            }
        ...
        case 'routine':
            ...
            if(config.metric === 'period_incomplete' && config.routinePeriod) {
                const filteredTasks = incompleteTasks.filter(task => task.period === config.routinePeriod);
                value = filteredTasks.length.toString();
                details = filteredTasks.map(t => ({ title: t.title, label: t.startTime || 'Any time'}));
            } else { // 'total_incomplete'
                value = incompleteTasks.length.toString();
                details = incompleteTasks.map(t => ({ title: t.title, label: t.period || 'General'}));
            }
            break;
        case 'finances':
            const monthlyIncome = getMonthlyIncome(incomeSettings);
            const expenses = financialAccounts.filter(acc => acc.type === 'expense');
            const totalExpenses = expenses.reduce((sum, item) => sum + convertCurrency(item.amount, item.currency, incomeSettings.currency), 0);
            ...
            if (config.metric === 'total_debt') {
                const totalDebtValue = financialAccounts
                    ...
                    .reduce((sum, item) => sum + convertCurrency(Math.abs(item.amount), item.currency, incomeSettings.currency), 0);
        ...
        case 'time':
            ...
            const weeklyEntries = timeTrackingEntries.filter(e => new Date(e.date) >= weekStart);
            if(config.metric === 'total_gaming') {
                const totalHours = weeklyEntries.filter(e => e.activityType === 'game').reduce((acc, e) => acc + e.hours, 0);
                value = `${totalHours.toFixed(1)} hours`;
            }
            if(config.metric === 'total_apps') {
                const totalHours = weeklyEntries.filter(e => e.activityType === 'app').reduce((acc, e) => acc + e.hours, 0);
                value = `${totalHours.toFixed(1)} hours`;
            }
            break;
        default:
...
export function DynamicDashboardCard({ card, allData, onClick }: DynamicDashboardCardProps) {
    ...
    return (
      <Collapsible open={isOpen} onOpenChange={setIsOpen} asChild>
        <Card className={cn("cursor-pointer hover:shadow-md transition-shadow flex flex-col", styles.card)} onClick={handleCardClick}>
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        {hasDetails && (
                            <CollapsibleTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground -mr-2" data-collapsible-trigger>

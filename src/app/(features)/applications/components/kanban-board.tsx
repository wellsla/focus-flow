...
const statusColumns: ApplicationStatus[] = ['Wishlist', 'Applied', 'Interviewing', 'Offer', 'Rejected'];

const renderApplicationLog = (log: DailyLog) => (
    <div className="space-y-4">
        {log.applications.length > 0 ? log.applications.map(appLog => (
        ...
        )) : <p className="text-muted-foreground text-center">No application data logged for this day.</p>}
    </div>
);


export function KanbanBoard({ dailyLogs, applications, setApplications }: { dailyLogs: DailyLog[], applications: JobApplication[], setApplications: Dispatch<SetStateAction<JobApplication[]>> }) {
    const [selectedApplication, setSelectedApplication] = useState<JobApplication | null>(null);
    const [isFormOpen, setIsFormOpen] = useState(false);

    const handleFormSubmit = (applicationData: JobApplication) => {
        if (selectedApplication) {
            setApplications(prev => prev.map(app => app.id === selectedApplication.id ? { ...app, ...applicationData, id: app.id } : app));
        } else {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                 <div>
                    <h1 className="text-2xl font-bold font-headline tracking-tight">Job Application Tracker</h1>
            ...
            <FormDialog
                ...
                description={selectedApplication ? 'Update the details of your job application.' : 'Track a new job application. Fill in the details below.'}
                triggerButton={null}

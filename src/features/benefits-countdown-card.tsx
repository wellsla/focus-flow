...
export function BenefitsCountdownCard({ endDate }: BenefitsCountdownCardProps) {
  // Avoid server/client time divergence by initializing after mount.
  const [remainingTime, setRemainingTime] = useState<ReturnType<
    typeof calculateRemainingTime
  > | null>(null);

  useEffect(() => {
    // Defer initial state update to avoid synchronous setState warning.
  ...
  const getUrgencyStyle = () => {
    const daysLeft = remainingTime?.days ?? 0;
        ...
        {remainingTime && remainingTime.totalSeconds > 0 ? (
          <div
            className={cn("text-2xl font-bold", styles.value, {
              "animate-pulse": urgency === "critical",
            })}
          >
            {remainingTime.days}d {remainingTime.hours}h {remainingTime.minutes}
            m {remainingTime.seconds}s
          </div>
        ) : (
          <div className={cn("text-2xl font-bold", styles.value)}>
            {remainingTime ? "Benefit period has ended." : "Calculating time…"}
          </div>
        )}
